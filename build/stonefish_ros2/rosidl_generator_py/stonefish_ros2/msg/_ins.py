# generated from rosidl_generator_py/resource/_idl.py.em
# with input from stonefish_ros2:msg/INS.idl
# generated code does not contain a copyright notice


# Import statements for member types

import builtins  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_INS(type):
    """Metaclass of message 'INS'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('stonefish_ros2')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'stonefish_ros2.msg.INS')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__ins
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__ins
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__ins
            cls._TYPE_SUPPORT = module.type_support_msg__msg__ins
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__ins

            from geometry_msgs.msg import Vector3
            if Vector3.__class__._TYPE_SUPPORT is None:
                Vector3.__class__.__import_type_support__()

            from std_msgs.msg import Header
            if Header.__class__._TYPE_SUPPORT is None:
                Header.__class__.__import_type_support__()

            from stonefish_ros2.msg import NEDPose
            if NEDPose.__class__._TYPE_SUPPORT is None:
                NEDPose.__class__.__import_type_support__()

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class INS(metaclass=Metaclass_INS):
    """Message class 'INS'."""

    __slots__ = [
        '_header',
        '_latitude',
        '_longitude',
        '_origin_latitude',
        '_origin_longitude',
        '_pose',
        '_pose_variance',
        '_altitude',
        '_body_velocity',
        '_rpy_rate',
    ]

    _fields_and_field_types = {
        'header': 'std_msgs/Header',
        'latitude': 'double',
        'longitude': 'double',
        'origin_latitude': 'double',
        'origin_longitude': 'double',
        'pose': 'stonefish_ros2/NEDPose',
        'pose_variance': 'stonefish_ros2/NEDPose',
        'altitude': 'double',
        'body_velocity': 'geometry_msgs/Vector3',
        'rpy_rate': 'geometry_msgs/Vector3',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['std_msgs', 'msg'], 'Header'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['stonefish_ros2', 'msg'], 'NEDPose'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['stonefish_ros2', 'msg'], 'NEDPose'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['geometry_msgs', 'msg'], 'Vector3'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['geometry_msgs', 'msg'], 'Vector3'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        from std_msgs.msg import Header
        self.header = kwargs.get('header', Header())
        self.latitude = kwargs.get('latitude', float())
        self.longitude = kwargs.get('longitude', float())
        self.origin_latitude = kwargs.get('origin_latitude', float())
        self.origin_longitude = kwargs.get('origin_longitude', float())
        from stonefish_ros2.msg import NEDPose
        self.pose = kwargs.get('pose', NEDPose())
        from stonefish_ros2.msg import NEDPose
        self.pose_variance = kwargs.get('pose_variance', NEDPose())
        self.altitude = kwargs.get('altitude', float())
        from geometry_msgs.msg import Vector3
        self.body_velocity = kwargs.get('body_velocity', Vector3())
        from geometry_msgs.msg import Vector3
        self.rpy_rate = kwargs.get('rpy_rate', Vector3())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.header != other.header:
            return False
        if self.latitude != other.latitude:
            return False
        if self.longitude != other.longitude:
            return False
        if self.origin_latitude != other.origin_latitude:
            return False
        if self.origin_longitude != other.origin_longitude:
            return False
        if self.pose != other.pose:
            return False
        if self.pose_variance != other.pose_variance:
            return False
        if self.altitude != other.altitude:
            return False
        if self.body_velocity != other.body_velocity:
            return False
        if self.rpy_rate != other.rpy_rate:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def header(self):
        """Message field 'header'."""
        return self._header

    @header.setter
    def header(self, value):
        if __debug__:
            from std_msgs.msg import Header
            assert \
                isinstance(value, Header), \
                "The 'header' field must be a sub message of type 'Header'"
        self._header = value

    @builtins.property
    def latitude(self):
        """Message field 'latitude'."""
        return self._latitude

    @latitude.setter
    def latitude(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'latitude' field must be of type 'float'"
            assert value >= -1.7976931348623157e+308 and value <= 1.7976931348623157e+308, \
                "The 'latitude' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._latitude = value

    @builtins.property
    def longitude(self):
        """Message field 'longitude'."""
        return self._longitude

    @longitude.setter
    def longitude(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'longitude' field must be of type 'float'"
            assert value >= -1.7976931348623157e+308 and value <= 1.7976931348623157e+308, \
                "The 'longitude' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._longitude = value

    @builtins.property
    def origin_latitude(self):
        """Message field 'origin_latitude'."""
        return self._origin_latitude

    @origin_latitude.setter
    def origin_latitude(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'origin_latitude' field must be of type 'float'"
            assert value >= -1.7976931348623157e+308 and value <= 1.7976931348623157e+308, \
                "The 'origin_latitude' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._origin_latitude = value

    @builtins.property
    def origin_longitude(self):
        """Message field 'origin_longitude'."""
        return self._origin_longitude

    @origin_longitude.setter
    def origin_longitude(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'origin_longitude' field must be of type 'float'"
            assert value >= -1.7976931348623157e+308 and value <= 1.7976931348623157e+308, \
                "The 'origin_longitude' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._origin_longitude = value

    @builtins.property
    def pose(self):
        """Message field 'pose'."""
        return self._pose

    @pose.setter
    def pose(self, value):
        if __debug__:
            from stonefish_ros2.msg import NEDPose
            assert \
                isinstance(value, NEDPose), \
                "The 'pose' field must be a sub message of type 'NEDPose'"
        self._pose = value

    @builtins.property
    def pose_variance(self):
        """Message field 'pose_variance'."""
        return self._pose_variance

    @pose_variance.setter
    def pose_variance(self, value):
        if __debug__:
            from stonefish_ros2.msg import NEDPose
            assert \
                isinstance(value, NEDPose), \
                "The 'pose_variance' field must be a sub message of type 'NEDPose'"
        self._pose_variance = value

    @builtins.property
    def altitude(self):
        """Message field 'altitude'."""
        return self._altitude

    @altitude.setter
    def altitude(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'altitude' field must be of type 'float'"
            assert value >= -1.7976931348623157e+308 and value <= 1.7976931348623157e+308, \
                "The 'altitude' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._altitude = value

    @builtins.property
    def body_velocity(self):
        """Message field 'body_velocity'."""
        return self._body_velocity

    @body_velocity.setter
    def body_velocity(self, value):
        if __debug__:
            from geometry_msgs.msg import Vector3
            assert \
                isinstance(value, Vector3), \
                "The 'body_velocity' field must be a sub message of type 'Vector3'"
        self._body_velocity = value

    @builtins.property
    def rpy_rate(self):
        """Message field 'rpy_rate'."""
        return self._rpy_rate

    @rpy_rate.setter
    def rpy_rate(self, value):
        if __debug__:
            from geometry_msgs.msg import Vector3
            assert \
                isinstance(value, Vector3), \
                "The 'rpy_rate' field must be a sub message of type 'Vector3'"
        self._rpy_rate = value
